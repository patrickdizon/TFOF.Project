  ...
  factory('$odataOperators', [function() {
	  ...
	  var trim = function(value) {
		...
		var convertOperator = function(from){
			...
			throw new Error("Operator "+ from+" not found");
		};
	...
	function() {
		...
		ODataValue.prototype.executeWithUndefinedType = function(isOdataV4) {
			if (angular.isString(this.value)) {
				...
				throw new Error("Unrecognized type of " + this.value);
			}
		};
		...
		ODataValue.prototype.executeWithType = function(isOdataV4){
			if(this.value === true || this.value === false){
				if(this.type.toLowerCase() === "boolean"){
					...
					throw new Error("Cannot convert bool ("+this.value+") into "+this.type);
				}
			}
			if(angular.isDate(this.value)){
				if(this.type.toLowerCase() === "decimal"){
					...
					throw new Error("Cannot convert date ("+this.value+") into "+this.type);
				}
			}
			if(angular.isString(this.value)){
				if(this.type.toLowerCase() === "guid"){
					...
					throw new Error("Cannot convert "+this.value+" into "+this.type);
				}
			}else if(!isNaN(this.value)){
				if(this.type.toLowerCase() === "boolean"){
					return !!this.value+"";
					...
					throw new Error("Cannot convert number ("+this.value+") into "+this.type);
				}
			}
			else{
				throw new Error("Source type of "+this.value+" to be conververted into "+this.type+"is not supported");
			}
		};
...
factory('$odataBinaryOperation', ['$odataOperators','$odataProperty','$odataValue',function($odataOperators,ODataProperty,ODataValue) {
	...
	var ODataBinaryOperation = function(a1,a2,a3){
		if(a1===undefined){
			throw new Error("The property of a filter cannot be undefined");
		}

		if(a2 === undefined){
			throw new Error("The value of a filter cannot be undefined");
		}
	...
	ODataBinaryOperation.prototype.or = function(a1,a2,a3){
		...
		if(a2!==undefined){
			...
			throw new Error("The object " +a1 +" passed as a parameter of the or method is not valid");
		}
	...
	ODataBinaryOperation.prototype.and = function(a1,a2,a3){
		...
		if(a2!==undefined){
			...
			throw new Error("The object " +a1 +" passed as a parameter of the and method is not valid");
		}
...
factory('$odataExpandPredicate', ['$odataPredicate', '$odataBinaryOperation', '$odataOrderByStatement', function (ODataPredicate, ODataBinaryOperation, ODataOrderByStatement) {
	...
	var ODataExpandPredicate = function (tableName, context) {
		if (tableName === undefined) {
			throw new Error("ExpandPredicate should be passed a table name but got undefined.");
		}

		if (context === undefined) {
			throw new Error("ExpandPredicate should be passed a context but got undefined.");
		}
	...
	ODataExpandPredicate.prototype.filter = function(operand1, operand2, operand3) {
		if (operand1 === undefined) throw new Error("The first parameter is undefined. Did you forget to invoke the method as a constructor by adding the 'new' keyword?");
	...
	ODataExpandPredicate.prototype.select = function (propertyName) {
		if (propertyName === undefined) {
			throw new Error("ExpandPredicate.select should be passed a property name but got undefined.");
		}
	...
	ODataExpandPredicate.prototype.expand = function (tableName) {
		if (tableName === undefined) {
			throw new Error("ExpandPredicate.expand should be passed a table name but got undefined.");
		}
	...
	ODataExpandPredicate.prototype.expandPredicate = function (tableName) {
		if (tableName === undefined) {
			throw new Error("ExpandPredicate.expandPredicate should be passed a table name but got undefined.");
		}
	...
	function(ODataProperty, ODataValue) {
		...
		var ODataMethodCall = function(methodName) {
			if (methodName === undefined || methodName === "")
				throw new Error("Method name should be defined");

			this.params = [];

			if (arguments.length < 2)
				throw new Error("Method should be invoked with arguments");
...
factory('$odataOrderByStatement', [function($odataOperators,ODataBinaryOperation,ODataPredicate) {
	...
	var ODataOrderByStatement = function(propertyName, sortOrder){
		if(propertyName===undefined){
			throw new Error("Orderby should be passed a property name but got undefined");
		}
...
factory('$odataPredicate', ['$odataBinaryOperation',function(ODataBinaryOperation) {
	...
	ODataPredicate.and = function(andStatements){
		...
		throw new Error("No statements specified");
	};
	...
	ODataPredicate.or = function(orStatements){
		...
		throw new Error("No statements specified for OR predicate");
	};
...
factory('$odataProvider', ['$odataOperators', '$odataBinaryOperation', '$odataPredicate', '$odataOrderByStatement', '$odataExpandPredicate',
	function($odataOperators, ODataBinaryOperation, ODataPredicate, ODataOrderByStatement, ODataExpandPredicate) {
		...
		ODataProvider.prototype.query = function(success, error) {
			if (!angular.isFunction(this.$$callback)) throw new Error("Cannot execute query, no callback was specified");
		...
		ODataProvider.prototype.single = function(success, error) {
			if (!angular.isFunction(this.$$callback)) throw new Error("Cannot execute single, no callback was specified");
			success = success || angular.noop;
			error = error || angular.noop;
		...
		ODataProvider.prototype.get = function(data, success, error) {
			if (!angular.isFunction(this.$$callback)) throw new Error("Cannot execute get, no callback was specified");
			success = success || angular.noop;
			error = error || angular.noop;
		...
		ODataProvider.prototype.count = function(success, error) {
			if (!angular.isFunction(this.$$callback)) throw new Error("Cannot execute count, no callback was specified");
			success = success || angular.noop;
			error = error || angular.noop;
			// The query string from this.execute() should be included even
			//  when fetching just a single element.
		...
		ODataProvider.prototype.expand = function(params) {
			if (!angular.isString(params) && !angular.isArray(params)) {
				throw new Error("Invalid parameter passed to expand method (" + params + ")");
			}
			if (params === "") {
				return;
			}
		...
		ODataProvider.prototype.select = function(params) {
			if (!angular.isString(params) && !angular.isArray(params)) {
				throw new Error("Invalid parameter passed to select method (" + params + ")");
			}
				...
				if (data) {
				  ...
				  if(angular.isArray(data) && forceSingleElement){
					if(data.length>0){
					  ...
					  throw new Error("The response returned no result");
					}
				  }
